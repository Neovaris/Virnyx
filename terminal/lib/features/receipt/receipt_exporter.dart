...
class ReceiptExporter {
  static Future<ReceiptExportResult> export(Sale sale) async {
    final dir = Directory('${Directory.systemTemp.path}\\virnyx_receipts');
    if (!await dir.exists()) {
      await dir.create(recursive: true);
    }

    final fileBase = (sale.receiptNo ?? sale.id).replaceAll(RegExp(r'[^\w\-]'), '_');

    final txtFile = File('${dir.path}\\$fileBase.txt');
    final htmlFile = File('${dir.path}\\$fileBase.html');

    await txtFile.writeAsString(ReceiptFormatter.format(sale));
    await htmlFile.writeAsString(ReceiptHtmlFormatter.format(sale));
